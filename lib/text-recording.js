...
export default class TR {
  ...
  addOtherCursor(start, stop, reversed, color) {
    var e = {
      'p': 'o',
      't': Date.now() - this.start_time,
      'd': [[start.row, start.column], [stop.row, stop.column]],
      'c': color
    }
    this.recording.push(e)
  }
  ...
  addEval(language, expression, range) {
    if (!this.ignoreChanges) {
      var e = {
        'p': 'e',
        't': Date.now() - this.start_time,
        'd': [[range.start.row, range.start.column], [range.end.row, range.end.column]],
        'c': expression,
        'l': language
      }
      this.recording.push(e)
