...
var AtomAdapter = function() {
  ...
  function AtomAdapter(atomInstance) {
    ...
    this.editor.decorateMarkerLayer(this.markerLayer, {type: 'cursor', class: 'my-red'})
    this.editor.decorateMarkerLayer(this.markerLayer, {type: 'highlight', class: 'selection'})
    this.otherCursors = new Map()
    this.ignoreChanges = false

    // this.editor.onDidStopChanging( (event) => {this.onChange(event)})
    this.editor.onDidChangeCursorPosition(() => {this.onCursorActivity()})
    this.editor.buffer.onDidChange((event) => {this.onChange(event)})
    this.atom.getCurrentWindow().on('blur', () => this.onBlur(this))
    this.atom.getCurrentWindow().on('focus', () => this.onFocus(this))
  }

  AtomAdapter.prototype.detach = function() {
    this.editor.removeListener('change', this.onChange);
    this.atom.getCurrentWindow().removeListener('blur', this.onBlur(this));
    this.atom.getCurrentWindow().removeListener('focus', this.onFocus(this));
    this.editor.onDidChange( (event) => {})
  ...
  AtomAdapter.prototype.onFocus = function (who) {
    who.trigger('focus')
  }

  AtomAdapter.prototype.onBlur = function (who) {
    if(who.editor.getSelectedBufferRange().isEmpty()){
      who.trigger('blur')
    }
  }
  ...
  AtomAdapter.prototype.setCursor = function (cursor) {
    ...
    if (cursor.position == cursor.selectionEnd) {
      this.editor.setCursorBufforPosition(start)
...
//teletype update selections for siteID
  AtomAdapter.prototype.setOtherCursor = function (cursor, color, clientId) {

    // console.log(cursor, color, clientId)
    if (typeof color !== 'string' || !color.match(/^#[a-fA-F0-9]{3,6}$/)) {
      return
    }
    ...
    var marker = this.otherCursors.get(clientId)
    var reversed = false
    if (cursor.selectionEnd > cursor.position) {reversed = true}
    var start = this.editor.buffer.positionForCharacterIndex(cursor.position)
    var stop = this.editor.buffer.positionForCharacterIndex(cursor.selectionEnd)
    if (marker) {
      marker.setBufferRange([[start.row, start.column], [stop.row, stop.column]], {reversed: reversed})
    } else {
      marker = this.markerLayer.markBufferRange([[start.row, start.column], [stop.row, stop.column]], {invalidate: 'never', reversed: reversed})
      marker.bufferMarker.onDidChange(({textChanged}) => {
        if (textChanged && marker.getBufferRange().isEmpty()) {
          marker.clearTail()
        }
      })
      this.otherCursors.set(clientId, marker)
    }
    marker
    if (cursor.position === cursor.selectionEnd) {marker.clearTail()}
  // } else {
  //   marker.destroy()
  ...
  AtomAdapter.prototype.invertOperation = function invertOperation(operation) {
    // TODO: Optimize to avoid copying entire text?
    operation.invert = this.getValue()
  };
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    if (!CodeMirror && !ace && !global.monaco && !atom) {
    ...
    if (CodeMirror && place instanceof CodeMirror) {
      ...
      //this needs work
    } else if (atom && place && atom.workspace.isTextEditor(place)) {
      this.atom_ = atom
      this.editor_ = place;
  ...
  Firepad.prototype.getText = function() {
    ...
    if (this.codeMirror_)
      ...
      return this.atom_.workspace.getActiveTextEditor().getText();
    else
  ...
  Firepad.prototype.setText = function(textPieces) {
    ...
    if (this.monaco_) {
      ...
      return this.atom_.workspace.getActiveTextEditor().setText(textPieces);
    } else {
  ...
  Firepad.prototype.insertText = function(index, textPieces) {
    ...
    utils.assert(!this.atom_, "Not supported for atom yet.");
