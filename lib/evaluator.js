...
export default class Evaluator {
  ...
  remoteEval(range, language, editor) {
    ...
    if (this.package.recording !== null) {
      this.package.recording.addEval(null, expression, range, language)
    }
    if (this.package.cibo !== null) {
      this.package.cibo.send(language, expression)
    }
    this.evalWithRepl(expression, range, false, language, editor)
  }
  ...
  localEval(evalType, copy, language, editor) {
    ...
    if (this.package.firebaseConnection !== null) { //check editor?
      this.package.firebaseConnection.addEval(range, language)
    }
  ...
  playbackEval(copy, language, editor, range, expression) {
    if (this.package.firebaseConnection !== null) { //check editor?
      this.package.firebaseConnection.addEval(range, language)
    }
    if (this.package.cibo !== null) {
